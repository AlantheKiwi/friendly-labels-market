      ...
      <div className="container-custom py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            {user ? <Button variant="outline" className="flex items-center gap-2" onClick={signOut} size="sm">
                <User className="h-4 w-4" />
                <span className="hidden sm:inline">Logout</span>
              </Button> : <Button variant="outline" className="flex items-center gap-2" size="sm" asChild>
                <Link to="/auth/login">
                  <LogIn className="h-4 w-4" />
                  <span className="hidden sm:inline">Client 
Login</span>
                </Link>
              </Button>}
        ...
        {mobileMenuOpen && <div className="md:hidden pt-4 pb-3 border-t border-gray-200 animate-fade-in">
            <nav className="flex flex-col space-y-4">
              ...
              <Link to="/printers" className="px-4 py-2 hover:bg-gray-50 rounded-md font-medium" onClick={() => setMobileMenuOpen(false)}>
                Printers
              </Link>
              <Link to="/contact" className="px-4 py-2 hover:bg-gray-50 rounded-md font-medium" onClick={() => setMobileMenuOpen(false)}>
                Contact
              </Link>
              {!user && <Link to="/auth/login" className="px-4 py-2 hover:bg-gray-50 rounded-md font-medium flex items-center gap-2" onClick={() => setMobileMenuOpen(false)}>
                  <LogIn className="h-4 w-4" />
                  Login
                </Link>}
            </nav>
